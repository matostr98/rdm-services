...
class PersonService:
    ...
    def _name_(self, person):
        names = []
        if person.sex == 1:
            file = open('./person/mens_name.txt', encoding="utf8")
        else:
            file = open('./person/girls_name.txt', encoding="utf8")
    ...
    def _surname_(self, person):
        ...
        surname = spolgloski[random.randint(0, len(spolgloski) - 1)] \
                  + samogloski[random.randint(0, len(samogloski) - 1)]
        # adding "koncowka" basic on sex
        if (person.sex == 1):
            surname = surname + koncowki_meskie[random.randint(0, len(koncowki_meskie) - 1)]
        else:
            surname = surname + koncowki_zenskie[random.randint(0, len(koncowki_zenskie) - 1)]
        return surname
